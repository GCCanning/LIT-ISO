...
namespace LitIso.UI.InGame
{
    ...
    /// <summary>
    /// What the HUD renders from. Codex's lane provides a real adapter that maps
    /// Inventory / Hotbar / player stats onto this; until then PlaceholderHudModel
    /// drives it so the bar previews standalone.
    /// </summary>
    public interface IGameHudModel
    {
        ...
        float Hunger01 { get; }   // 0..1
        float Xp01 { get; }       // 0..1 toward next level
        int Level { get; }
        event Action Changed;     // raise when any of the above changes
    }

    /// <summary>Demo data so the HUD is visible/skinnable before Codex binds real systems.</summary>
    ...
    /// vitals (health / hunger / XP+level) stacked at the bottom-left.
    ///
    ...
    ///
    /// Integration: Codex's FoundationBootstrap should create this and call Init(model)
    /// with an adapter over Inventory/Hotbar/stats, then retire the IMGUI FoundationHUD.
    /// </summary>
    ...
    public sealed class GameUIController : MonoBehaviour
    {
        ...
        static readonly Color XpCol       = new Color(0.35f, 0.65f, 0.95f, 1f);
        ...
        Image _healthFill, _hungerFill, _xpFill;
        Text  _levelText;

        /// <summary>Bind a real data model (Codex's adapter) and rebuild against it.</summary>
        ...
        {
            // Bottom-left vertical stack of three bars.
            ...
            col.sizeDelta = new Vector2(280f, 96f);

            _healthFill = Bar(col, "Health", 0, HealthCol, "bar_health_fill");
            _hungerFill = Bar(col, "Hunger", 1, HungerCol, "bar_hunger_fill");
            _xpFill     = Bar(col, "XP",     2, XpCol,     "bar_xp_fill", 14f);
        ...
        Image Bar(RectTransform parent, string name, int row, Color fillCol, string fillSkin, float h = 22f)
        {
            float gap = 6f;
            float y = -(row * (h + gap));

            var track = NewImage(parent, name + "Track", Spr("bar_track"), BarTrack);
            ...
            tr.sizeDelta = new Vector2(280f, h);
        ...
        void BuildHotbar(Transform parent)
        {
            ...
            bar.sizeDelta = new Vector2(totalW + 16f, slot + 12f);

            // Optional bar background behind the slots.
            var barBgSpr = Spr("bar_bg");
            if (barBgSpr != null)
            {
        ...
        {
            var frame = NewImage(parent, "Slot" + i, Spr("slot"), SlotBg);
            ...
            var hi = NewImage(fr, "Highlight", Spr("slot_selected"), SlotSelect);
            hi.type = Image.Type.Sliced;
            hi.raycastTarget = false;
            if (Spr("slot_selected") == null) hi.color = new Color(SlotSelect.r, SlotSelect.g, SlotSelect.b, 0.35f);
        ...
        void Refresh()
        {
            ...
            _healthFill.fillAmount = Mathf.Clamp01(_model.Health01);
            _hungerFill.fillAmount = Mathf.Clamp01(_model.Hunger01);
            _xpFill.fillAmount     = Mathf.Clamp01(_model.Xp01);
